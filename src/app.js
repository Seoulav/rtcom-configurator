  ...
  (()=>{
    ...
    let modalSlot=null;
    const frameFronts={'XDM-12':'output/design/assets/frames/xdm-12-front.webp','XDM-36':'output/design/assets/frames/xdm-36-front.webp'};
    ...
    function cardsViewV4(){
      ...
      const slotButton=slot=>{const c=slotCard(slot.id);return `<button type="button" class="rt-rack-slot ${c?'rt-rack-slot-filled':''}" data-slot="${slot.id}" aria-label="${esc(slot.label)}, ${c?`${c[0]} 장착됨 · 눌러서 변경`:'비어 있음 · 눌러서 카드 선택'}" title="${esc(slot.label)}${c?` · ${c[0]}`:''}"><span class="rt-rack-slot-no" aria-hidden="true">${shortLabel(slot)}</span>${c?`<img class="rt-faceplate" src="${cardAsset(c[0])}" alt="">`:'<span class="rt-rack-slot-add" aria-hidden="true">+</span>'}</button>`};
      ...
      const manual=photoPages[model]?`<details class="rt-manual-ref"><summary>매뉴얼 후면 사진과 비교하기 · 국문 매뉴얼 p.${photoPages[model]}</summary><figure class="rt-manual-rear"><img src="output/design/assets/${model.toLowerCase()}-rear.jpg" alt="${model} 실제 후면 사진 (매뉴얼)" loading="lazy"></figure></details>`:'';
      const layoutText=count?`${layout==='vt'?'상단':'왼쪽'} 입력 ${count}슬롯 / ${layout==='vt'?'하단':'오른쪽'} 출력 ${count}슬롯 · 카드당 4채널`:'슬롯 구성 검토용 논리 도식';
      return heading('03 / CARD SLOTS','후면의 빈 슬롯을 눌러 카드를 장착하세요.',`${esc(model)} · ${layoutText}`)+`<div class="rt-config-stage"><section class="rt-rack-canvas"><div class="rt-rack-toolbar"><div><span class="rt-eyebrow">REAR VIEW</span><h3>${esc(model)}</h3></div><div class="rt-frame-count"><span><b>${inputCards}</b> / ${inputSlots.length} INPUT</span><span><b>${outputCards}</b> / ${outputSlots.length} OUTPUT</span></div></div><div class="rt-rack-scroll"><div class="rt-rack rt-rack-${layout}" style="--rt-rack-columns:${columns};--rt-rack-rows:${Math.max(1,Math.ceil(inputSlots.length/columns))*2};--rt-bank-slots:${count}"><span class="rt-rack-ear" aria-hidden="true"></span><div class="rt-rack-body">${bank('input',inputSlots)}${bank('output',outputSlots)}<div class="rt-rack-psu" aria-hidden="true"><strong>RTCOM</strong><span>${esc(model)}</span><i></i><small>CONTROL</small><i></i><small>POWER</small></div></div><span class="rt-rack-ear" aria-hidden="true"></span></div></div>${layout==='h'?'':'<p class="rt-rack-scroll-hint">좌우로 밀어서 후면 전체를 볼 수 있습니다.</p>'}${count?'':'<p class="rt-stage-warning">이 프레임은 제조사 후면 도면과 카드 허용표를 확보하기 전까지 논리 도식으로 표시합니다. 물리 설치 위치로 사용하지 마세요.</p>'}${manual}</section>${configurationSummary()}</div>${cardChoiceModal()}`;
    }
