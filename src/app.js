  ...
  (()=>{
    ...
    function cardsViewV3(){
      ...
      const slotButton=slot=>{const c=slotCard(slot.id);return `<button type="button" class="rt-hardware-slot ${c?'rt-installed':''}" data-slot="${slot.id}" aria-pressed="${state.slot===slot.id}"><span class="rt-hardware-slot-label">${slot.label}</span>${c?`<img src="${cardAsset(c[0])}" alt="${c[0]} 카드"><strong>${c[0]}</strong><small>${c[2]} CHANNEL</small>`:'<span class="rt-empty-slot"><b>+</b> EMPTY</span>'}</button>`};
