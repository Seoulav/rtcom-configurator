  ...
  (()=>{
    ...
    function linksViewV3(){
      ...
      return heading('04 / EXTENDERS','카드에 연결할 전송 장비를 확인하세요.',remote.length?`HDBaseT·광 카드 ${remote.length}장에 기본 전송기를 연결했습니다. 필요하면 벽부형이나 채널 수를 바꾸세요.`:'HDBaseT·광 카드를 장착하면 연동 전송기가 자동으로 연결됩니다.')+`<div class="rt-link-list">${remote.length?remote.map(block).join(''):empty}</div>${powerNotice()}${lineup}`;
    }
