  ...
  (()=>{
    ...
    function cardAsset(id){return state.family==='XDM'&&id?`output/design/assets/cards/${id}.jpg`:assets[state.family]}
    const cardTips={
    ...
    function chassisView(){const f=families[state.family];const xdmFeature=state.family==='XDM'?'<div class="rt-xdm-feature-note"><span>EDID · LED TIP</span><strong>커스텀 해상도도 설계할 수 있습니다.</strong><p>개선 펌웨어 기준으로 비표준 입력을 원본 패스스루(LED) 또는 4K 업스케일(모니터) 경로로 나눠 검토합니다. 출고용 EDID 주입 조건은 제조사 확인이 필요합니다.</p></div>':'';return heading('02 / CHASSIS','구성의 중심이 될 섀시를 선택하세요.',`${state.family} 제품군 · 메인프레임 선택`)+`<div class="rt-split"><div class="rt-model-list"><div class="rt-model-list-intro">프레임을 고르면 후면 슬롯과 호환 카드를 확인할 수 있습니다.</div>${f.models.map((m,i)=>`<button class="rt-model" data-model="${m}" type="button" aria-pressed="${state.model===m}"><span><strong>${m}</strong><small>${f.modelNotes[i]}</small></span><span class="rt-radio" aria-hidden="true">${state.model===m?'✓':''}</span></button>`).join('')}</div><div class="rt-preview"><div class="rt-preview-label"><span class="rt-eyebrow">${state.family} SERIES</span><span class="rt-pill">제품군 미리보기</span></div><h3>${state.model||'섀시를 선택해 주세요'}</h3><div class="rt-photo"><img src="${assets[state.family]}" alt="${state.family} 제품군 전체 라인업 사진"></div><p class="rt-caption">${f.name}</p><div class="rt-line"></div><p class="rt-caption">사진은 제품군 라인업입니다.<br>${state.family==='XDM'?'다음 화면에서 국문 매뉴얼의 모델별 후면과 슬롯 배치를 확인할 수 있습니다.':'다음 화면의 슬롯은 배치 방식을 보여주는 예시입니다.'}</p>${xdmFeature}</div></div>`}
    ...
    function cardsViewV3(){
      ...
      const slotCounts={'XDM-12':3,'XDM-20':5,'XDM-36':9,'XDM-72':18,'XDM-144':36,'XDM-216':54};
      const confirmed=state.family==='XDM'&&Boolean(slotCounts[state.model]);
      const vertical=['XDM-72','XDM-144','XDM-216'].includes(state.model);
      const columns=state.model==='XDM-12'?1:Math.min(18,inputSlots.length);
      const slotButton=slot=>{const c=slotCard(slot.id);return `<button type="button" class="rt-hardware-slot ${c?'rt-installed':''}" data-slot="${slot.id}" aria-pressed="${state.slot===slot.id}"${c?` title="${c[0]} · ${cardTip(c[0])}"`:''}><span class="rt-hardware-slot-label">${slot.label}</span>${c?`<img src="${cardAsset(c[0])}" alt="${c[0]} 카드"><strong class="rt-slot-card-name">${c[0]}</strong><small>${c[2]} CHANNEL</small>`:'<span class="rt-empty-slot"><b>+</b> EMPTY</span>'}</button>`};
      const bank=(direction,items)=>`<section class="rt-slot-side rt-${direction}-side"><div class="rt-side-title"><strong>${direction==='input'?'INPUT':'OUTPUT'}</strong><span>${items.length} SLOTS</span></div><div class="rt-side-grid">${items.map(slotButton).join('')}</div></section>`;
      const layoutText=confirmed?`${vertical?'상단':'왼쪽'} 입력 ${inputSlots.length}슬롯 / ${vertical?'하단':'오른쪽'} 출력 ${outputSlots.length}슬롯 · 카드당 4채널`:'슬롯 구성 검토용 초안';
      const photoPages={'XDM-12':7,'XDM-20':8,'XDM-36':8,'XDM-72':9,'XDM-144':10};
      const rearPhoto=photoPages[state.model]?`<figure class="rt-manual-rear"><img src="output/design/assets/${state.model.toLowerCase()}-rear.jpg" alt="${state.model} 실제 후면 사진"><figcaption>국문 매뉴얼 p.${photoPages[state.model]} · ${state.model} 실제 후면</figcaption></figure>`:'';
      const wosTip=selectedSlot.dir==='output'?'<div class="rt-output-tips"><div class="rt-pro-tip rt-quad-tip"><span>4분할</span><div><strong>XDM-HOS100 · QUAD VIEW</strong><p>일반 HDMI 4채널 출력 또는 정사분할 화면 구성에 활용할 수 있습니다.</p></div></div><div class="rt-pro-tip"><span>활용 TIP</span><div><strong>XDM-WOS100 · 4 LAYER</strong><p>4개 레이어를 조합해 월 프로세서처럼 화면을 연출할 수 있습니다.</p></div></div></div>':'';
      const selectedSummary=selectedCard?`<div class="rt-selected-card-summary"><img src="${cardAsset(selectedCard[0])}" alt="${selectedCard[0]} 카드"><div><span>CURRENT CARD</span><strong>${selectedCard[0]}</strong><p>${cardTip(selectedCard[0])}</p></div></div>`:`<div class="rt-selected-card-summary rt-selected-card-empty"><div><span>SELECT A CARD</span><strong>${selectedSlot.label}</strong><p>아래 목록에서 이 슬롯에 장착할 카드를 선택하세요.</p></div></div>`;
      return heading('03 / CARD SLOTS','장비 후면의 빈 슬롯을 눌러 카드를 장착하세요.',`${state.model} · ${layoutText}`)+`<div class="rt-configurator-stage"><section class="rt-frame-area"><div class="rt-frame-header"><div><span class="rt-eyebrow">REAR CARD CONFIGURATION</span><h3>${esc(state.model)}</h3><p>${confirmed?'매뉴얼의 후면 슬롯 구조를 반영한 구성':'제조사 후면 자료 확인 필요'}</p></div><div class="rt-frame-count"><span><b>${inputCards}</b> / ${inputSlots.length} INPUT</span><span><b>${outputCards}</b> / ${outputSlots.length} OUTPUT</span></div></div>${rearPhoto}<div class="rt-rear-scroll"><div class="rt-rear-panel ${slotList.length>6?'rt-rear-dense':''} ${vertical?'rt-rear-vertical':''}" style="--rt-slot-columns:${columns}"><div class="rt-rack-handle" aria-hidden="true"></div>${bank('input',inputSlots)}<div class="rt-rear-center"><strong>RTCOM</strong><span>${esc(state.model)}</span><i aria-hidden="true"></i><small>CONTROL</small><i aria-hidden="true"></i><small>POWER</small><div class="rt-vent" aria-hidden="true"></div></div>${bank('output',outputSlots)}<div class="rt-rack-handle" aria-hidden="true"></div></div></div><div class="rt-rear-caption"><span>${vertical?'↑ 상단 입력 카드 영역':'← 왼쪽 입력 카드 영역'}</span><b>클릭 가능한 후면 구성</b><span>${vertical?'하단 출력 카드 영역 ↓':'오른쪽 출력 카드 영역 →'}</span></div><div class="rt-channel-summary"><span>입력 카드 <b>${inputCards}장</b> · <b>${t.input}채널</b></span><span>출력 카드 <b>${outputCards}장</b> · <b>${t.output}채널</b></span></div></section><aside class="rt-card-drawer"><div class="rt-drawer-head"><div><span class="rt-eyebrow">${selectedSlot.dir==='input'?'INPUT':'OUTPUT'} CARD</span><h3>${selectedSlot.label} 카드 선택</h3></div>${selectedCard?'<span class="rt-selected-dot">장착됨</span>':''}</div>${selectedSummary}${wosTip}<div class="rt-card-options">${families[state.family][selectedSlot.dir].map(c=>`<button type="button" class="rt-card-option ${['XDM-WOS100','XDM-HOS100'].includes(c[0])?'rt-featured-card':''}" data-card="${c[0]}" aria-pressed="${state.placements[state.slot]===c[0]}"><span class="rt-add-icon">${state.placements[state.slot]===c[0]?'✓':'+'}</span><span class="rt-card-option-copy"><strong>${c[0]}${cardBadge(c[0])}</strong><small>${c[1]} · ${c[2]}채널</small><span class="rt-card-tip">${cardTip(c[0])}</span></span><img src="${cardAsset(c[0])}" alt="${c[0]} 카드 이미지"></button>`).join('')}</div><button type="button" class="rt-button rt-remove-card" data-action="remove" ${selectedCard?'':'disabled'}>이 슬롯의 카드 제거</button></aside></div><div class="rt-stage-note">${confirmed?`첨부된 XDM 국문 매뉴얼의 ${state.model} 슬롯 설명${rearPhoto?'과 실제 후면 사진':''}을 기준으로 구성했습니다. 카드를 선택하면 같은 위치의 슬롯에 실제 카드 이미지가 장착됩니다.`:'현재 프레임의 실제 슬롯 수와 배치는 제조사 자료 확인 후 확정해야 합니다.'}</div>`;
    }
