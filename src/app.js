  ...
  (()=>{
    ...
    function chassisViewV2(){
      ...
      const tile=(m,i)=>{
        ...
        return `<button type="button" class="rt-chassis-card ${front?'':'rt-chassis-card-generic'}" data-model="${m}" aria-pressed="${selected}"><span class="rt-chassis-visual"><img src="${front||assets[state.family]}" alt="${front?`${m} 전면 ${frontDrawings.has(m)?'도면':'사진'}`:`${state.family} 제품군 사진`}" loading="lazy"></span><span class="rt-chassis-body"><span class="rt-chassis-name"><strong>${m}</strong><span class="rt-radio" aria-hidden="true">${selected?'✓':''}</span></span><small>${esc(f.modelNotes[i]||'')}</small><span class="rt-chassis-specs">${specs}</span></span></button>`;
      };
    ...
    function linksViewV3(){
      ...
      const empty=`<div class="rt-empty rt-link-empty"><strong>현재 구성에는 HDBaseT·광 카드가 없습니다.</strong><p>${state.family==='SPX'?'SPX-COS12(CATx 출력) 카드를 장착하면 SPX-RX가 자동으로 연결되고 여기서 채널 수를 바꿀 수 있습니다.':'XDM-CIS100·COS100(HDBaseT) 또는 XDM-FIS100·FOS100(광) 카드를 장착하면 CTR100·FT101·FR101이 자동으로 연결되고 여기서 바꿀 수 있습니다.'}</p><button type="button" class="rt-button" data-jump="2">카드 슬롯으로 돌아가기</button></div>`;
