...
const appRoutes=new Set(['/products','/products/','/tools/matrix-configurator','/tools/matrix-configurator/']);
const server=http.createServer((req,res)=>{
  ...
  if(appRoutes.has(pathname))file=path.join(root,'index.html');
  ...
  fs.stat(file,(error,stat)=>{
    ...
    if(appRoutes.has(pathname)){const html=fs.readFileSync(file,'utf8').replace('<head>','<head><base href="/">');res.end(html);return}
