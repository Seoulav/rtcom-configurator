...
## 8. 후속 결정 — 기타 알티컴 장비 재게시 (2026-09-26)

- 계기: 사용자 "기타 알티컴 장비들도 다시 원래대로 올릴까 싶어"
- 검토한 선택지
  1. 구성기 아래에 "기타 알티컴 장비" 영역을 직접 만들기 — 시안 제작(아래)
  2. AV Portal의 제품 데이터를 읽어 와서 보여 주기(자료는 AV Portal 한 곳에서 관리)
  3. 구성기 전용 유지 + AV Portal 링크만
- **결정: 2번으로 한다. 단, AV Portal이 아직 완성되지 않아 AV Portal 완성 후에 진행한다.** AV Portal과 같은 목록을 이 사이트에 따로 만드는 방식(1번)은 사양·사진·단종 정보를 두 곳에서 고쳐야 하고, 0.7에서 제품 라이브러리를 뺀 이유(중복)가 다시 생기므로 하지 않는다.
- 그 전까지: 구성기에서 쓰는 전송기(XDM 6종, VDM 4종, SPX-RX)는 지금처럼 전송기 단계에서 보여 준다.

### 시안 구성 (재게시할 때 참고)

- 위치: 구성기 아래 한 화면 안(페이지 이동 없음 → 0.6의 이미지 깨짐·뒤로가기 404가 생기지 않음)
- 분류 탭: 전체 25 / 소형 매트릭스·멀티뷰어 3 / 분배기·스위처 9 / 전송기 13
- 카드: 제품 사진, 모델명, 한 줄 설명, 핵심 사양 3줄(휴대폰 2줄), 카탈로그 쪽 번호, "AV Portal ↗" 링크. 구성기 카드와 연동되는 전송기에는 "연동 · VDM CIS4-U·COS4-U" 같은 표시
- 배치: 데스크톱 4열, 휴대폰 2열
- 대상 25종
  - 소형 매트릭스·멀티뷰어: HS-88MX, QMS-44UX, QMS-88UX
  - 분배기·스위처: HD-D102U, HD-D104U, HD-D108U, HDS-21U, HDS-42MU, HD-13U, HD-104U, HD-108U, HD-210U
  - 전송기: XDM-CTR100/PSE, XDM-CT103/CR103, XDM-FT101/FR101, CT104-U/CR104-U, FT101-U/FR101-U, CT101-U/CR101-U, CT102-U/CR102-U, CT103-U-H/CR103-U, FT102-U/FR102-U, FT103-U-H/FR103-U, MR-4S, OBHD-2C, OBUX-1C
- 데이터·사진 원본: 0.6 커밋 `e263514`의 `src/product-catalog.js`(사양·카탈로그 쪽 번호)와 `output/design/assets/library/*.jpg`(사진 28장). VDM 전송기 사진은 `output/design/assets/extenders/vdm-*.webp`(0.18).
- 재게시 때 필요한 것: AV Portal의 제품 목록 데이터 형식(파일 위치·필드) 또는 장비별 제품 페이지 주소. "표기 검토 필요"였던 CT103-U-H·FT102-U·FT103-U-H(카탈로그 오기, 근거 문서 E23~E25)의 표시 방식도 그때 정한다.
