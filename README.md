...
- XDM-12는 입력 카드 3장과 출력 카드 3장을 실제 슬롯처럼 선택하며 각 카드를 4채널로 계산.
...
`npm test`로 3+3 슬롯 제한, 카드당 4채널, 파일 복원, JSON 마이그레이션과 BOM을 검사합니다. JSON 스키마 3은 XDM-12의 여섯 슬롯을 저장하며, 스키마 1·2의 기존 슬롯 이름은 새 이름으로 변환합니다. 프레임레이트·거리·개별 포트 요구 입력은 현재 검토 목적에서 제외했습니다. XDM-12 외 모델의 실제 슬롯 수와 제조사 근거가 없는 카드·TX/RX 관계는 `UNVERIFIED`로 유지합니다.
